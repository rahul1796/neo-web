...
class Report:
    ...
    def DownloadBatchStatusReport(user_id,user_role_id,customer_ids,contract_ids,contract_status,batch_status,from_date,to_date):
        try:
            ...
            df=pd.DataFrame(response)
            if len(df.index)>0:
                df=df[[ 'Practice_Name','Customer_Name','Contract_Code','Contract_Name','Center_Name','Product_Name','Bu_Name','Region_Name','Batch_Code','Course_Code','Status','Actual_Start_Date','Actual_End_Date','Unit_Rate','E_Ratio','C_Ratio','P_Ratio','Batch_Enrolled','Batch_Dropped','Batch_Certified','Certification_Distribution','Batch_Placement','Placement_Count_Rr','Ojt_Completed','Filtered_Enrolled','Filtered_Dropped','Filtered_Certified','Filtered_Certification_Distribution','Filtered_Placed','Filtered_Ojt_Completed','Excess_No','Excess_Revenue','Overall_Revenue','Mon_Revenue']]
