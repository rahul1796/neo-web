def create_report(user_id, user_role_id, Role, Date, file_name):
    ...
    try:
        ...
        #print(columns)
        df= df[['User_Name', 'Email', 'User_Role_Name', 'Product', 'Conversion_Criteria', 'U_M_Target', 'E_M_Count', 'Mper', 'P_M_Target', 'E_M_Count', 'Mpper',
        ...
        default_column = ['User', 'Email', 'Role', 'Product', 'Conversion Criteria']
        ...
        for col_num, value in enumerate(first_row):
            worksheet.merge_range(0, 5+col_num*6, 0, 10+col_num*6, value, header_format)
        for col_num, value in enumerate(second_row):
            worksheet.merge_range(1, 5+col_num*3, 1, 7+col_num*3, value, header_format)
        for col_num, value in enumerate(third_row):
            worksheet.write(2, 5+col_num, value, header_format)
