        ...
        try:
            ...
            for row in out:
                ...
                if intervention_id!=[]:
                    ...
                    b.append(row[80])
                    temp += '\n' + "({},'SAE',GETDATE(),{},1),".format(row[0],"(select u.user_id from users.tbl_users as u left join users.tbl_user_details as ud on ud.user_id=u.user_id where u.is_active=1 and ud.email like trim('{}'))".format(row[82]))

                if ProjectType==2:
                    query += '\n' + quer8.format(row[84],row[85],row[86],row[87],row[88],row[89],row[90],row[91],row[92],row[93],row[94],row[95],row[96],row[97],row[98],row[99],row[100],row[0])
